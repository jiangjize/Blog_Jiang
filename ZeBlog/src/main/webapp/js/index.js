   ...
   form.on('submit(login)', function (data) {
        ...
        setTimeout(function () {
          ...
          /*  if (data.field.account != 'lyblogscn' || data.field.password != '111111') {
                ...
                layer.closeAll('page');
                setTimeout(function () {*/
                    location.href = "/UserServlet?act=Login&username="+uname+"&password="+upwd;
           /*     }, 1000);
            }*/
        }, 400);
  ...
  /*  form.on('submit(login)', function (data) {
        ...
        setTimeout(function () {
            ...
            $.ajax({
                ...
                data: "username=" + data.field.account + "&password=" + data.field.password + "",
                success: function (date) {
                    var dataObj=eval("("+date+")");//转换为json对象
                    if (dataObj.result == "true") {
                        layer.msg('登陆成功，正在跳转......', {icon: 6});
                        layer.closeAll('page');
                        location.href = "/LoginServlet?action=logininfo";
                    } else {
                        layer.msg('账号或者密码错误', {icon: 5});
                    }
                }
